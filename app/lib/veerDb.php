...
class veerDb {   
    ...
    /**
     * Query Builder: Products for One Specific Attribute with Images
     * 
     * @not: later: comments, tags, categories; 
     */
    ...
    /**
     * Query Builder: Pages for One Specific Attribute with Images 
     * 
     * @not: later: categories, tags, comments;
     */
    public function attributeOnlyPagesQuery($siteId, $id, $queryParams)
    {
        $id = explode('.', $id);
        $id = $id[1];
        return Page::whereHas('attributes',function($q) use($id) { $q->where('attributes_id','=',$id); })
                ->with(array(                
                    'images' => function($query) { $query->orderBy('id', 'asc')->take(1); }                    
                    ))->sitevalidation($siteId)->excludeHidden()->orderBy('created_at','desc')
                            ->take($queryParams['take_pages'])->skip($queryParams['skip_pages'])->get();         
    ...
    /**
     * Query Builder: Top Level Attributes
     * 
     * @not: categories, tags, comments <-- too many; 
     * separate: products & pages <-- too many
     * - we should include twice the same statement as OR has primary priority over AND
     */
    public function attributeIndexQuery($siteId, $id = null, $queryParams = array())
    {
        return Attribute::withTrashed()->whereHas('products',function($q) use($siteId) { $q->sitevalidation($siteId); })
                ->whereRaw('`attributes`.`deleted_at` is null')
                ->orWhereHas('pages',function($q) use($siteId) { $q->sitevalidation($siteId); })
                ->whereRaw('`attributes`.`deleted_at` is null')->get();       
    }  
     ...
     * 
     * @not: tags, comments, attributes, categories
     * separate: products & pages
     */
    protected function imageShowQuery($siteId, $id, $queryParams)
    {
        return Image::where('id','=',$id)->withTrashed()->whereHas('products',function($q) use($siteId) { $q->sitevalidation($siteId); })
                ->whereRaw('`images`.`deleted_at` is null')
                ->orWhereHas('pages',function($q) use($siteId) { $q->sitevalidation($siteId); })
                ->whereRaw('`images`.`deleted_at` is null')->where('id','=',$id)
                ->orWhereHas('categories',function($q) use($siteId) { $q->where('sites_id','=',$siteId); })
                ->whereRaw('`images`.`deleted_at` is null')->where('id','=',$id)->first();
    }
    
    
    /**
     * Query Builder: Products for One Specific Images
     * 
     * @not: later: comments, tags, categories, attributes; 
     */
    ...
    /**
     * Query Builder: Pages for One Specific Images 
     * 
     * @not: later: categories, tags, comments, attributes;
     */
    public function imageOnlyPagesQuery($siteId, $id, $queryParams)
    {
        return Page::whereHas('images',function($q) use($id) { $q->where('images_id','=',$id); })
                ->sitevalidation($siteId)->excludeHidden()->orderBy('created_at','desc')
                            ->take($queryParams['take_pages'])->skip($queryParams['skip_pages'])->get();         
    }  
     ...
     * 
     * @not: categories?, pages?, subproducts, parenproducts, tags, attributes, images, comments, downloads, userlists
     */
