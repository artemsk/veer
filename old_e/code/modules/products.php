...
class products {
 ...
 Debug::log();
        // prices - ������ ���, priors - ������ �����������, attrs - attr_id /��� ��� ���������� ��������/, client - id �������, type - prd, grp
        // grp_alll_nnn - ������ ��� ����� - nnn ��� �������� ��������, ?order_discount - �����. ������ �� ������� ��� ������ �� ����� � �����
        // ?admin_customers - ������ customers_type, customers_discount, 
        // ?customers_discount_expire -- ������ �� ������� ��� ������. ������ �� ����� �������
        
        $client_status = "user";
        $current_discount = 0; // ���������
        if ($client > 0) { // ������ �������: ����� (distr), ����� (guest), ������� (opt), �������������� ������ ()
        ...
        return $result_price; // out: 0 - ����, 1 - ������ (0-6), 2 - �������������� ����
    }
...
Debug::log();
        // ���������� �������� � ����, �������� � ��� ��� ������ �������
        // grp_nnn - id ������� � ������, grp_price - ������������� ��������� ���� ��� ��������� � ������� product_prices, 
        // attrs - ��������� ��������, client - id
        ...
        if (mysql_num_rows($v4) > 0) {
            ...
            do {
                ...
                foreach ($v2_flip[$v3] as $v2_flip_k => $v2_flip_v) {
                    ...
                    $summ_grp_price = $summ_grp_price + $return_price[0]; // ����� ��������� ������
                    
                    $summ_grp_price_starter = $summ_grp_price_starter + $return_price[2]; // �������������� ���������
                    
                    $return_arr['priors'][trim($v3)] = $priors; // ������ � �������������, � �� �����?
    ...
    function product_attrs($nnn, $type, $priors="") { // nnn, type=grp,prd, priors[prd_id] - ����������
   Debug::log();     
        ...
        if(mysql_num_rows($attrs)>0) { unset($flag_4_dominant, $flag_4_dominant_already, $flag_4_dominant_pnnn);
            do { $main['attrs'][$attrs2['products_nnn']][$attrs2['attr_name']][$attrs2['attr_val']]['attr_nnn']=$attrs2['nnn'];
                 ...
                 if((isset($flag_4_dominant_pnnn[$attrs2['products_nnn']])&&isset($flag_4_dominant_already)&&!isset($flag_4_dominant[$attrs2['attr_name']]))&&$attrs2['attr_dominant']=="1") {
                     ...
                     } // ���� ����������� �������� ��� �������, �� ������ ����������� ���� �� ����� (����. nnn, prd_id, attr_name, attr_dominant)
                 ...
                 $main['attrs'][$attrs2['products_nnn']][$attrs2['attr_name']][$attrs2['attr_val']]['price']=@$return_price[0]; // ��������� ��������� ������ �� ����������
    ...
    function collect_products($i, $type = "cat", $group_img_flag = "0", $onlynums_flag = "0", $startfrom=0, $sorttype="dat", $sortdirection="DESC") {
        Debug::log();
        // ������: type=nnn / cat / srch / main / new / attr / keyw / nnn_connects / nnn_array
        // ����������� � ��������, ������, �������, ���� ��������: �������, �������������, �� ���� ������, �� ����� ����������
        // nnn_connects - ��������������� ������ ����� ���� �� ����� ���������!
    ...
    function collect_products_full($i) { // �������� ��� ��������� ������ �� ������
        ...
        if(@$return_price[0]>0) { $main['price']=$return_price[0]; $main['price_star']=@$return_price[1]; $main['price_starter']=@$return_price[2]; } // ���� ����� ������
        ...
        if(@$return_arr['price_grp_currency']>0) { $main['price_grp_currency']=$return_arr['price_grp_currency']; } // false - ���� ����. ����
        if(@$return_arr['price_starter']>0) { $main['price_starter']=$return_arr['price_starter']; } // false - ���� ����. ����
        if(@$return_arr['price_grp_currency_starter']>0) { $main['price_grp_currency_starter']=$return_arr['price_grp_currency_starter']; } // false - ���� ����. ����
	...
	$main['shop_cat_basketsep'][$sql_cats2['shop_cat']]=$sql_cats2['basket_separate'];
        ...
        } else { $ii=$sel['grp_nnn']; $priors=@$return_arr['priors']; } // $return_arr['priors'][] - ������� ����� ������� product_group
        
        $return_attr=$this->product_attrs($ii,$sel['type'],$priors); // LEVEL 3->LEVEL 2
        // out -> [prd_id][attr_name][attr_val] attr_nnn, attr_type, attr_descr, change, attr_dominant, price, price_starter

        if(count(@$return_attr)>0) { $main=$main+$return_attr; } // ���������� ������ �� ���������� � ����������
    ...
    function collect_attrs($attrs,$prd_type,$grp_nnn="",$currency,$catshop_currency,$currency_price,$skip_flag="0") {
        ...
        //// attrs- array, prd_type - prd,grp, grp_nnn - ��� ������, currency - ���� ������, catshop_currency - ���� ��������
        // currency_price - ������������ �� ���� ���� ������, skip_flag - ��� ������, ���� 1 - ������ ������������� ���� � �������� ������ �� ������
        ...
        if(count($arr['attrs'])>0) {
            ...
            foreach($vvv as $kkkk=>$vvvv) { 
                                if(count($vvv)>1) { // ���� ��� ������, �� ���� �� ����� �������� � � ������� �� ����������
                                    $price_attr=""; 
                                    // $kkkk - ��������: attr_nnn! attr_type!, attr_descr-, change!, attr_dominant!, price!
                                    ...
                                    if($vvvv['attr_type']=="choose") {
                                            ...
                                            if($arr['type']!="grp") { 
                                                ...
                                                // ���� ������, �� ����������� ������� ����� ����� ����� ������ (price_attr_currency) 
                                                // � ����������� (currency_price)
                                                if(@$price_attr_currency>0) { // ���� ����� ���� ���, �� �������� �������� 0 (������, ��� ������)
                                                $price_attr=$price_attr_currency-$arr['currency_price']; // �������
                                                if($price_attr>=0) { $price_attr_format="+"; } // ��������� + ��� �����
                                                                            } else { $price_attr=0; } // ������
                                            ...
                                            if($price_attr<0||$price_attr>0) {
                                                ...
                                                } else { $price_attr=0; $str_opt2=$kkkk; $str_opt=$str_opt2; } // ���� 0, �� ������ �� ������
                                      ...
                                      if($vvvv['attr_type']=="descr"||$vvvv['attr_type']=="choose") { // �������� ������ ��� �������� � attr_id
...
Debug::log();
        ...
        if($kogda2>0) { $arr['kogda']="����� �������� ����� ".$kogda2." ��. (".date("d.m.Y",$arr['kogda']).")"; } else {
        $arr['kogda']=""; }}
        ...
        if($arr['type']=="grp") { $arr['img']=img_mini(@$arr['img'],$showtype, $key_arr); } else { // ���� ������
        // ���� ������� �����
        if($showtype=="brief"||$showtype=="tiny"||$showtype=="tiny_bsk"||$showtype=="compare") { // �� ����� ��������
       ...
       // �����������, ������ � �.�,
       // price_currency
       if(isset($arr['shop_cat_priority'])) { $catshop_currency=$arr['shop_cat_currency'][$arr['shop_cat_priority']]; } else { // ��� �� ����������, � ���� ��� �� �� ��������
       ...
       if($arr['type']!="grp"||($arr['type']=="grp"&&!isset($arr['price_grp_currency']))) { // ��� ������� ��� ����� � ������������� �����
       ...
       } else { $arr['currency_price']=$arr['price_grp_currency']; $arr['currency_price_starter']=$arr['price_grp_currency_starter']; } // � ������ �� ��� ������� �������
       ...
       if($arr['status']=="predzakaz") { $baskettxt=BASKET_PREDZAKAZ_TXT; } // � ����������� �� ������� ������ ����� �������

        // shop_cat, status
        // shop_cat -> �������, shop_cat_priority -> ������������ ������, shop_cat_onlyshop -> ������������ �������
        // shop_cat_onlyshop_arr -> ��� �������� + connected_2_current - ������ ����� � �������
        ...
        if(@$arr['shop_cat_onlyshop_arr'][$cat_status_k]['nnn']!=SHOP_NNN) { // ����� �� ��������� � ��� ��������

             if(@$arr['shop_cat_onlyshop_arr'][$cat_status_k]['connected_2_current']==""&&array_key_exists('connected_2_current',@$arr['shop_cat_onlyshop_arr'][$cat_status_k])) { // ������� ������ �� ��������
                $cat_status=1; } else {  // ������� ������ ��������
        ...
        // ���������� ��������� ��� ������� ��� ���, ���� ���, �� ��������������
        // TODO: ������ ����� ������ ����������� �������� ��������� �� �������� �������� (1-2-3-4) + �����. ��������?
        if((@$cat_status=="1"||@$cat_status=="2")&&@$arr['shop_cat_onlyshop_arr'][$cat_status_k]['remote_addr']!="") { // ��������� ������ ���
        ...
        $arr['basket_link']="<a href=".$arr['shop_cat_onlyshop_arr'][$cat_status_k]['remote_addr']."product/".$key_arr."/add2cart>".$baskettxt."</a>"; } // TODO: ����������
        ...
        if($cat_status!="1"&&$arr['status']=="temp") { $arr['basket_link']=BASKET_NO_ORDER; $arr['price_formated']=""; } // TODO: ���������� temp, predzakaz, free
        ...
        if($showtype=="tiny"||$showtype=="tiny_bsk") { // ������� ����� ��� ��������
        ...
        if($showtype=="tiny") {
			...
			//
                ...
                $tiny_arr['shop_cat_status']=@$arr['shop_cat'][@$arr['shop_cat_onlyshop']['nnn']]; // � ������ �������� ��������� �����
                $tiny_arr['shop_cat_nazv']=@$arr['shop_cat_nazv'][@$arr['shop_cat_onlyshop']['nnn']]; // ����. ������, � �� �������
    ...
    function show_basket() { 
        ...
        if(@$sql=="") { $bask_out="{###}������ ������. ".BASKET_EMPTY."."; } else {
    ...
    // ������� ��� ������� [1]
    function show_basket_collect_info($p_id) { // �������� ������ �� p_id �� ������� // <-> collect_products_full!
Debug::log();
                    $v2=$this->collect_products_full($p_id); if($v2['type']=="") { return false; }
                    $v2=$this->show_product($v2, $p_id, 'tiny_bsk');

                    if($v2['status']!="buy") { unset($v2, $bsk[$p_id], $b_out[$p_id]); $form_out=""; } else { // ��������� �����, ������� ���� ������ buy

                    $form_out=""; if($v2['type']=="grp") { // ������
                    ...
                    } else { if(count($v2['attr_choose'])>0) { // ��������� �����:
    ...
    function show_basket_attrs($p_id, $n, $sql3, $attr_form, $attr_form_shifr, $type="prd") {
        Debug::log();
                        ...
                        $select_f=$attr_form_shifr[$shifr]; // �������, ������� ���� ��������
        ...
        $bsk_all['total']['weight']=0; 
            ...
            foreach($bsk as $k=>$v) {
                ...
                $bsk_all[$k]['current_price']=0;
                        ...
                        //if($bsk[$k][$kk]['data']['price_star']=="1") { // <s>���� ������� ��������� ����, �� ������� ��� pure</s>
    ...
    function sort_products($resort, $type="dat", $direction="desc", $limityes="0", $sortarr=array()) {
        ...
        foreach($resort as $k=>$v) { 
           ...
           if($type=="shopcat_limit") { $resort2[$resort[$k]['smart_sort']][$k]=$k; } else { $kaunt_limit++; } // ��� shopcat_limit ������ ��������� ������ � ��������� ������� �������
    ...
    function show_comments($i=SHOP_NNN, $limit="50") { // �������� ���?
        Debug::log();
    ...
    function add2list($post) { // list_name, prd_id - ���������� � �����, �� �� � �������. 
        Debug::log();
        ...
        if(!isset($_SESSION['customers_id'])) { $cid="0"; $temp_session=session_id(); } else { $cid=$_SESSION['customers_id']; $temp_session=""; }
           ...
           if (mysql_num_rows(mysql_kall("SELECT list_name FROM " . DB_PREFIX . "customers_basket_lists 
            ...
            unset($_SESSION['customers_lists']);
             $_SESSION['send_login_message'] = "����� �������� � ������ \"" . $post['list_name'] . "\"";
        }
    }
    ...
    function add2basket($post="",$from="indoor") { // prd_id, attr_NNN_NNN_NNN=VALUE___NNN_ - ���������� � �������, ��� ����������� � ��� ����� � ��
  Debug::log();
        if($from=="outdoor") { 
             if(isset($_COOKIE[SHOP_NNN.'_timeout_add2cart'])&&(time()<@$_COOKIE[SHOP_NNN.'_timeout_add2cart'])) {
                 $_SESSION['send_login_message']="����������, ��������� �������. ���������� ��� ���."; return; } else {
    ...
    function update_basket($post="") {
        ...
        // ������������ ������ ���������������� ������            
        if(@filemtime(MAINURL_5."/temp/delete_all_temporary_baskets.txt")<(time()-(60*60*24*3))) { // 3 ���
        ...
        if(isset($post['attrs_descr'][$k])) { $unser=@unserialize(stripslashes($post['attrs_descr'][$k])); }
            ...
            // $k."=> q:".$v."<br>"; // debug
            if(isset($rearr[$k])) {
            foreach($rearr[$k] as $kk=>$vv) { $v=$v-1; // ��������� ������ �� ����������
            ...
            mysql_call("INSERT INTO ".DB_PREFIX."customers_basket_lists (customers_id, list_name, prd_id, quantity, date_added, temp_session)
                ...
                foreach($vv as $kkk=>$vvv) { foreach($vvv as $kkkk=>$vvvv) { // ��������� ��������
                $kkk2=explode("/",$kkk); 
                if(trim($vvvv)!=""&&@$unser[$kkkk][$vvvv]>0) { // �������� �������!
                ...
                if(isset($c_lists4[COMPARE_LIST_NAME])&&@$c_lists4[COMPARE_LIST_NAME]>=1) {
                    ...
                    if(mysql_num_rows($prds)>0) {
                        ...
                        // ����� ���� ��� ���������?
                            ...
                            $compare_out['out_of_list'][$prds2['prd_id']]="<form method='post' action='".MAINURL_2."/user/done/'><input type=hidden name='prdid' value='".$prds2['prd_id']."'><input type=submit name='outoflist' value='������'></form>";
                            // TODO: ��������� ����� �������� ������� � forms
                            unset($compare_pre_out[$prds2['prd_id']]);
                        } while($prds2=mysql_fetch_assoc($prds));


                    }} else { $compare_out="";  $_SESSION['send_login_message']="��� ������� � ������ ��� ���������"; }
    ...
    function add_review($post) { // review
        Debug::log();
           ...
           $_SESSION['send_login_message']="����� ���������� ���������� ������, ���������, ���������� ��� <b>".($_COOKIE[SHOP_NNN.'_timeout_review']-time())."</b> ���.";  } else {
         ...
         if(!isset($post['rate_yn'])) { $post['rate_yn']="vote_y"; } 
           ...
           $_SESSION['send_login_message']="����� ��������.";         
               }} else { $_SESSION['send_login_message']="����������� �� ��������, �.�. ��� ���."; }
                } else { $_SESSION['send_login_message']="�������� ����������� ���."; }
                } else { $_SESSION['send_login_message']="����������� ��� �� ������."; }
        } // review rate
    ...
    function add_review_only_rate($post) { // review
        ...
        $_SESSION['send_login_message']="����������, ��������� ��� <b>".($_COOKIE[SHOP_NNN.'_timeout_rate_yn']-time())."</b> ���.";  } else {
           $timeout0=time()+(60*COMMENTS_TIMEBREAK);
           setcookie(SHOP_NNN."_timeout_rate_yn",$timeout0,"0","/",MAINURL_4);
         if(isset($_SESSION['customers_id'])) { $c_id=$_SESSION['customers_id']; } else { $c_id=0; }
         if(isset($post['vote_y_x'])) { $post['rate_yn']="vote_y"; $msg="��� ���������� �����."; } else { $post['rate_yn']="vote_n"; $msg="��� �� ���������� �����."; }
           mysql_call("INSERT INTO ".DB_PREFIX."products_reviews (products_nnn, dat, avtor, customers_id, txt, rate, ".$post['rate_yn'].")
    ...
    function product_listing($prd, $look = "", $group_flag = "0", $used_income = "", $inline = LIMIT_PRD_IN_LINE, 
        ...
        //// prd - ������ � ��������, inline - ���-�� ������� � �����, $maxprd - ������������ ����������
        //// group_flag - �����������, $look - ������ ���� ���. ��������
        //// used_income - �������������� ���, ������� �������, group_by - ����� �� ��������,
        //// skip_show_product - �� ������������ �������� show_product,
        //// start_from - ?, main_page_flag = ����� ��� �������, ������ ��������; arrflag - ������ �������������� ������!

        $prd_filename = MAINURL_5 . "/template/" . TEMPLATE . "/product_list.php";
        $templ_prd = get_include_contents($prd_filename); // ###������_����
        $templ_prd_types = explode("[####]", $templ_prd); //  0, 1, 2, 3, 4; 5 -> ���� �������, ���� ������.
        $templ_prd3 = "";
    ...
    function product_listing_group($prd, $templ_prd4, $templ_prd4_type, $templ_prd4_countimg, $templ_prd_types, $look, $inline, $maxprd, $grp_by="") {
        Debug::log();
            ...
            if((count($prd['group_by_shop'])>1&&$grp_by!="cat")||$grp_by=="shop") { // ���������� �� ���������
            $k3="group_by_shop";  } else { // �� ��������
            $k3="group_by_cat"; }
            if(isset($prd[$k3])) {
