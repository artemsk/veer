...
class pages {
        ...
        foreach ($filevars as $k => $v) {
            if (count($filevars[$k]) > 0) {
                if ($filevars[$k]['error'] <> 0) {
                    ...
                    $str_upl = file_upload($filevars[$k], MAINURL_5 . "/upload/", "jpg"); // ���������� ������� upload
    ...
    // ������ �������: ����: list, last, news. limit ������ ��� last. main - ����� last � news ��� ������� ��������!
            ...
            if ($_SESSION['number1'] == mb_strtolower($post['vercode'])) {
                if ($post['txt'] != "") {
                    if (isset($_COOKIE[SHOP_NNN . '_timeout']) && (time() < @$_COOKIE[SHOP_NNN . '_timeout'])) {
                        $_SESSION['send_login_message'] = "����� ���������� ���������� �����������, ���������, ���������� ��� <b>" . ($_COOKIE[SHOP_NNN . '_timeout'] - time()) . "</b> ���.";
                    } else {
    ...
    function prepare_news($news_list, $autocollapse = 15, $newsimp = 3, 
        ...
        foreach ($news_list as $n1 => $n2) {
            ...
            if (trim($news_list[$n1]['txt_full']) != "" && $n4 <= $autocollapse) {
                $n3.="<span class=\"" . $css_arr[2] . "\"><a href=" . MAINURL . "/page/" . $n1 . ">���������</a></span>";
            }
            if ($n4 > $newsimp) {
