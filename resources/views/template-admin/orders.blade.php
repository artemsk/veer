...
<h1>Orders :{{ array_get($data, 'counted.active', 0) }}<small> @if(array_get($data, 'counted.archived', 0) > 0)<a href="{{ route("admin.show", array("orders", "filter" => "archive", "filter_id" => true)) }}">~{{ array_get($data, 'counted.archived', 0) }}</a>&nbsp;@endif 
